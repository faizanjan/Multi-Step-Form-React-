...
let Step2 = ({isMonthly, switchMonthly, setPlan, activePlan, setStep}) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-muted my-2 mb-5">
        You have the option of isMonthly or yearly billing
      </p>
      ...
      >
        {plans.map((plan, index) => (
          <Plan
            ...
            isMonthly={isMonthly}
            isChecked={plan.id===activePlan.id}
            setPlan={setPlan}
          />
        ))}
      </div>
      ...
      <div className="form-check form-switch d-flex flex-row justify-content-center align-items-center my-5 py-4">
        <span className={isMonthly? classesForMonthlyAndYearly+ " fw-bold" : classesForMonthlyAndYearly}>
          Monthly
        </span>
        <input
          ...
          onClick={()=>{switchMonthly(!isMonthly)}}
          defaultChecked={!isMonthly}
        />
        <span className={!isMonthly? classesForMonthlyAndYearly+ " fw-bold" : classesForMonthlyAndYearly}> Yearly</span>
      </div>
    </form>
  );
};
